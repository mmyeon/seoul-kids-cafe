...
describe('buildSeoulApiUrl', () => {
  it('API 키와 엔드포인트로 올바른 URL을 생성해야 한다', () => {
    ...
    expect(url).toBe('http://openAPI.seoul.go.kr:8088/TEST_KEY/json/ChildCareInfo/1/1000/');
  });

  it('페이지 범위를 반영한 URL을 생성해야 한다', () => {
    const url = buildSeoulApiUrl('MY_KEY', 1001, 2000);
    expect(url).toBe('http://openAPI.seoul.go.kr:8088/MY_KEY/json/ChildCareInfo/1001/2000/');
  });
});
